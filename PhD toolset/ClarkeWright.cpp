...
c_ClarkeWright::InsertionPoint c_ClarkeWright::CheapestInsertionPoint( int request, FacilityClass Facility, vector<NodeClass> NodeList ) {
    InsertionPoint insertion_point;
    
    float MaxCost = floatbigM;
    
    int erase_pos = -1;
    int node_insert = request;
    int route_insert = -1;
    int pos_insert = -1;
    float insertion_cost = 0;
    
    for (int j = 0; j < Facility.RouteList.size(); j++ ) {
        
        float sum_demands = 0;
        for ( int k = 0; k < Facility.RouteList[j].size(); k++ ) {
            sum_demands = sum_demands + NodeList[Facility.RouteList[j][k]].Demand;
        }
        sum_demands = sum_demands + NodeList[request].Demand;
        
        if (sum_demands <= Facility.Capacity) {
            for ( int k = 0; k < Facility.RouteList[j].size(); k++ ) {
                FacilityClass TempFacility = Facility;
                TempFacility.RouteList[j].insert(TempFacility.RouteList[j].begin() + k, request);
                insertion_cost = CalculateFacilityCosts(TempFacility, NodeList) - CalculateFacilityCosts(Facility, NodeList);
                if (insertion_cost < MaxCost) {
                    MaxCost = insertion_cost;
                    route_insert = j;
                    pos_insert = k;
                    node_insert = request;
                }
            ...
            FacilityClass TempFacility = Facility;
            TempFacility.RouteList[j].insert(TempFacility.RouteList[j].end(), request);
            insertion_cost = CalculateFacilityCosts(TempFacility, NodeList) - CalculateFacilityCosts(Facility, NodeList);
            if (insertion_cost < MaxCost) {
                MaxCost = insertion_cost;
                route_insert = j;
                pos_insert = Facility.RouteList[j].size();
            }
        }
        insertion_point.Route = route_insert;
        insertion_point.Pos = pos_insert;
        insertion_point.Node = node_insert;
        insertion_point.Cost = MaxCost;
    }
    
    if (insertion_point.Route == -1) {
        FacilityClass TempFacility = Facility;
        TempFacility.RouteList.push_back(vector<int>());
        Facility.RouteList[Facility.RouteList.size() - 1].push_back(request);
        insertion_point.Cost = CalculateFacilityCosts(TempFacility, NodeList) - CalculateFacilityCosts(Facility, NodeList);
    }
...
vector<vector<int>> c_ClarkeWright::GenerateAll( int size, vector<vector<int>> all, vector<int> current, vector<int> remaining ) {
    if (current.size() == size) {
        all.push_back(current);
    } else {
        for(int i = 0; i < remaining.size(); i++) {
            vector<int> temp_remaining = remaining;
            vector<int> temp_current = current;
            temp_current.insert(temp_current.end(), remaining[i]);
            temp_remaining.erase(temp_remaining.begin() + i);
            all = GenerateAll(size, all, temp_current, temp_remaining);
        }
    }
...
FacilityClass c_ClarkeWright::VRPSS2( FacilityClass Facility, vector<NodeClass> NodeList ) {
    FacilityClass ReturnFacility = Facility;

    vector<int> stochastic_nodes =  Facility.ServiceNodes;
    
    for (int i = 0; i < Facility.FirstStageNodes.size(); i++) {
        for (int j = 0; j < stochastic_nodes.size(); j++) {
            if( Facility.FirstStageNodes[i] == stochastic_nodes[j]) {
                stochastic_nodes.erase(stochastic_nodes.begin() + j);
            }
        }
    }
    ...
    vector<int> ReoptimizeRoutes;

    while (stochastic_nodes.size() > 0) {
        float MaxCost = floatbigM;
        vector<InsertionPoint> BestInsertions;
        
        vector<vector<InsertionPoint>> Seq;
        
        for ( int i = 0; i < stochastic_nodes.size(); i++ ) {
            insertion_point = CheapestInsertionPoint(stochastic_nodes[i], ReturnFacility, NodeList);
            BestInsertions.push_back(insertion_point);
        }
        ...
        for ( int i = 0; i < BestInsertions.size(); i++ ) {
            if (BestInsertions[i].Cost < MaxCost) {
                MaxCost = BestInsertions[i].Cost;
                best = i;
            }
        }
        ...
        if (BestInsertions[selected].Route != -1) {
            ReturnFacility.RouteList[BestInsertions[selected].Route].insert(ReturnFacility.RouteList[BestInsertions[selected].Route].begin() + BestInsertions[selected].Pos, BestInsertions[selected].Node);
            stochastic_nodes.erase(stochastic_nodes.begin() + selected);
        } else {
            ReturnFacility.RouteList.push_back(vector<int>());
            ReoptimizeRoutes.push_back(ReturnFacility.RouteList.size() - 1);
            ReturnFacility.RouteList[ReturnFacility.RouteList.size() - 1].push_back(stochastic_nodes[0]);
            stochastic_nodes.erase(stochastic_nodes.begin());
        }
    }
    
    for ( int i = 0; i < ReoptimizeRoutes.size(); i++) {
        ReturnFacility.RouteList[ReoptimizeRoutes[i]] = DetermineTour(NodeList, ReturnFacility.RouteList[ReoptimizeRoutes[i]], Facility.LocationX, Facility.LocationY);
    }
...
vector<int> c_ClarkeWright::ConvexHull(vector< NodeClass > NodeList, vector<int> Route, float LocationX, float LocationY) {
    vector<int> ReturnRoute;
    
    int n = (int)Route.size() + 1, k = 0;
    ...
    for (int i = 0; i < n - 1; i++) {
        RN[0] = NodeList[Route[i]].LocationX;
        RN[1] = NodeList[Route[i]].LocationY;
        RN[2] = Route[i];
        RL3.push_back(RN);
    };
    ...
    for(int i = 0; i < CRL3.size(); i++) {
        if (CRL3[i][2] == 0) {
            for (int j = i + 1; j < CRL3.size(); j++) {
                ReturnRoute.push_back(CRL3[j][2]);
            }
            for (int j = 0; j < i; j++) {
                ReturnRoute.push_back(CRL3[j][2]);
            }
    ...
    if (ReturnRoute.size() == 0) {
        for (int i = 0; i < CRL3.size(); i++) {
            ReturnRoute.push_back(CRL3[i][2]);
        }
    }
...
FacilityClass c_ClarkeWright::ClarkeWright(std::vector< NodeClass > NodeList, FacilityClass Facility) {
    switch (SolutionMethod) {
        ...
        //case 8: return VRP(Facility, NodeList);
...
//    cout << " Routes calculated -> ";
    for (int i = 0; i < TempFacility.RouteList.size(); i++) {
        TempFacility.RouteList[i] = DetermineTour(NodeList, TempFacility.RouteList[i], TempFacility.LocationX, TempFacility.LocationY);
    };
...
//    cout << " Routes Optimized." << endl;
    ...
    Facility.RouteList = TempFacility.RouteList;
...
vector<vector<int>> c_ClarkeWright::GurobiIntegerL(std::vector< NodeClass > NodeList, FacilityClass Facility) {
//
//
//    GRBEnv env = GRBEnv();
//
...
//
//    int p = ConstrainedFacility.RouteList.size();
//
...
//        y[j] = model.addVar(demands[j], 9999999999, 0.0, GRB_CONTINUOUS, s); //from i to j with vehicle k
//    };
//
...
//                if (i != j) {
//                    GRBLinExpr Condv = 0;
//                    Condv += y[i] + (demands[j] * x[i][j][k]) - (60*(1-x[i][j][k]));
...
//        GRBLinExpr NodeEnterFirst = 0;
//        int index1;
//        int r1 = ConstrainedFacility.RouteList[i][0];
...
//        }
//        for (j = 0; j < NodeAmount; j++) {
//            NodeEnterFirst += x[j][index1][i];
//        };
//        model.addConstr(NodeEnterFirst, GRB_EQUAL, 1.0);
//    }
//
//
//    for (i = 0; i < ConstrainedFacility.RouteList.size(); i++) {
...
//            RouteList.push_back(vector<int>());
//            for (int i = 1; i < NodeList.size(); i++) {
//                RouteList[RouteList.size()-1].push_back(i);
//            }
//        }
//
//        result = RouteList;
...
vector<vector<int>> c_ClarkeWright::GurobiAFSS(int minva, ScenarioClass sc, std::vector< NodeClass > NodeList, FacilityClass Facility, float *obj, vector<vector<int>> Constraints) {
//    GRBEnv env = GRBEnv();
//
//    FacilityClass ConstrainedFacility = Facility;
//
...
//    }
//
//    for (int c_int = 0; c_int < Constraints.size(); c_int++) {
//        if (Constraints[c_int][0] == 0) {
//            int node1 = Constraints[c_int][1];
//            int node2 = Constraints[c_int][2];
//
//            int p_node1 = 0;
//            int p_node2 = 0;
//            for (int h = 0; h < Facility.ServiceNodes.size(); h++) {
//                if (node1 == Facility.ServiceNodes[h]) {
//                    p_node1 = h + 1;
//                }
//                if (node2 == Facility.ServiceNodes[h]) {
//                    p_node2 = h + 1;
//                }
//            }
//            for (int k = 0; k < p; k++) {
...
//                for (i = 0; i < NodeAmount; i++) {
//                    if (p_node1 != i) {
//                        customerenter += x[i][p_node1][k];
//                    }
//                }
//                GRBLinExpr customerenter2 = 0;
//                for (i = 0; i < NodeAmount; i++) {
//                    if (p_node2 != i) {
//                        customerenter2 += x[i][p_node2][k];
//                    }
//                }
//                model.addConstr(customerenter, GRB_EQUAL, customerenter2);
//            }
//        } else {
//            int node1 = Constraints[c_int][1];
//            int node2 = Constraints[c_int][2];
...
//                    }
//                }
//                model.addConstr( (2 * customerenter  + 2 *  customerenter2 ), GRB_LESS_EQUAL, 2);
//            }
//        }
//    }
//
...
vector<vector<int>> c_ClarkeWright::GurobiAFSSLEQ(int minva, ScenarioClass sc, std::vector< NodeClass > NodeList, FacilityClass Facility, float *obj, vector<vector<int>> Constraints) {
//    GRBEnv env = GRBEnv();
//
//    FacilityClass ConstrainedFacility = Facility;
//
...
vector<vector<int>>  c_ClarkeWright::GurobiAltFormSecondStageK(std::vector< NodeClass > NodeList, FacilityClass Facility, float *obj,  int vehicle_amount) {
//    GRBEnv env = GRBEnv();
//
//    FacilityClass ConstrainedFacility = Facility;
//
...
FacilityClass c_ClarkeWright::GurobiSol(std::vector< NodeClass > NodeList, FacilityClass Facility) {
    ...
    ResultFacility.RouteList.clear();
    float sumdemands = 0;
    for (int i = 0; i < ResultFacility.ServiceNodes.size(); i++) {
        sumdemands += NodeList[ResultFacility.ServiceNodes[i]].Demand;
    }
    int vehicle_amount = ceil(sumdemands / ResultFacility.Capacity);
    while (ResultFacility.RouteList.size() == 0 && vehicle_amount <= ResultFacility.ServiceNodes.size()) {
        ResultFacility = GurobiSolOptions(NodeList, ResultFacility, false, vehicle_amount);
        vehicle_amount = vehicle_amount + 1;
    }
    if ( ResultFacility.RouteList.size() == 0) {
        ResultFacility = GurobiSolOptions(NodeList, ResultFacility, true, vehicle_amount);
    }
...
FacilityClass c_ClarkeWright::GurobiSolOptions(std::vector< NodeClass > NodeList, FacilityClass Facility, bool lessorequal, int vehicle_amount) {
//    FacilityClass ResultFacility = Facility;
//    GRBEnv env = GRBEnv();
//
//    GRBModel model = GRBModel(env);
...
//        int q = 0;
//
//        while (dd == false && RouteList.size() != 0) {
//            if (RouteList[q].size() == 0) {
//                RouteList.erase(RouteList.begin() + q);
//                q = 0;
//            }
...
vector<vector<int>> c_ClarkeWright::GurobiScenarioK(int minva, ScenarioClass sc, std::vector< NodeClass > NodeList, FacilityClass Facility, float *obj, vector<vector<int>> Constraints) {
//    GRBEnv env = GRBEnv();
//
//    FacilityClass ConstrainedFacility = Facility;
//
...
vector<vector<int>> c_ClarkeWright::GurobiScX(int minva, ScenarioClass sc, std::vector< NodeClass > NodeList, FacilityClass Facility, float *obj, vector<vector<int>> Constraints) {

    vector<vector<int>> ForbiddenRoutes;
    ...
    vector<vector<int>> x_temp;
    
    x_temp = GurobiScenarioTest(minva, sc, NodeList, Facility, obj, ForbiddenRoutes);
    ...
    for (int a = 0; a < Constraints.size(); a++) {
        if (Constraints[a][0] == 0) { //joint
            ...
            for (int j = 0; j < RL.size(); j++) {
                for (int k = 0; k < RL[j].size(); k++) {
                    if ( RL[j][k] == Constraints[a][1] ) {
                        r1 = j;
                        t1 = k;
                    }
                    if ( RL[j][k] == Constraints[a][2] ) {
                        r2 = j;
                        t2 = k;
                    }
                }
    ...
    for (int i = 0; i < Destroy.size(); i++) {
        ForbiddenRoutes.push_back(RL2[Destroy[i]]);
    }
    
    while (Destroy.size() > 0) {
        Destroy.clear();
        RL.clear();
        RL2.clear();
        x_temp = GurobiScenarioTest(minva, sc, NodeList, Facility, obj, ForbiddenRoutes);
...
vector<vector<int>> c_ClarkeWright::GurobiScenarioTest(int minva, ScenarioClass sc, std::vector< NodeClass > NodeList, FacilityClass Facility, float *obj, vector<vector<int>> ForbiddenRouteList) {
//    GRBEnv env = GRBEnv();
//
//    FacilityClass ConstrainedFacility = Facility;
//
...
//
//
//    for (i = 0; i < ForbiddenRouteList.size(); i++) {
//        int s = ForbiddenRouteList[i].size() - 1;
//
//        GRBLinExpr fr = 0;
//        GRBLinExpr sr = 0;
//        fr += x[0][ForbiddenRouteList[i][0]];
//        sr += x[ForbiddenRouteList[i][0]][0];
//        for (j = 0; j < s; j++) {
//             fr += x[ForbiddenRouteList[i][j]][ForbiddenRouteList[i][j+1]];
//             sr += x[ForbiddenRouteList[i][j + 1]][ForbiddenRouteList[i][j]];
//        }
//        fr += x[0][ForbiddenRouteList[i][s]];
//        sr += x[ForbiddenRouteList[i][s]][0];
//
//        model.addConstr(fr, GRB_LESS_EQUAL, s);
//        model.addConstr(sr, GRB_LESS_EQUAL, s);
//    }
//
//
//    for (i = 1; i < NodeAmount; i++) {
...
vector<vector<int>> c_ClarkeWright::RouteSplitter(std::vector< NodeClass > NodeList, FacilityClass Facility, vector<int> route) {
...
//        sumdemands += NodeList[route[i]].Demand;
//    }
//
//    int p = ceil(sumdemands / Facility.Capacity);
//
...
//    for (j = 0; j < NodeAmount; j++) {
//        string s = "Y_" + to_string(j);
//        b[j] = model.addVar(demands[j], 999999, 0.0, GRB_CONTINUOUS, s); //from i to j with vehicle k
//    };
//
//
//    //every customer should be visited by one vehicle (both entering and exiting
...
//        for (j = 1; j < NodeAmount; j++) {
//            if (i != j) {
//                GRBLinExpr CorrectDemand = 0;
//                CorrectDemand += b[i] + (distance[i][j] * x[i][j]) - (999999 * (1-x[i][j]));
//                model.addConstr(CorrectDemand, GRB_LESS_EQUAL, b[j]);
//            }
//        }
//    };
//
//    for ( i = 0; i < route.size() - 1; i++) {
//         model.addConstr(b[i+1], GRB_LESS_EQUAL, b[i+2]);
//    }
//
//    model.set(GRB_IntAttr_ModelSense, GRB_MAXIMIZE);
//
...
//                bool done = false;
//                RouteList.push_back(vector<int>());
//                RouteList[ RouteList.size()-1].push_back(route[i-1]);
//
//                t_I = i;
//                while (done == false) {
//                    done = true;
//                    for (int j = 1; j < NodeAmount; j++) {
//                        if (x[t_I][j].get(GRB_DoubleAttr_X) > 0.5) {
//                            RouteList[ RouteList.size()-1].push_back(route[j-1]);
//                            done = false;
//                            t_I = j;
//                        }
//                    }
