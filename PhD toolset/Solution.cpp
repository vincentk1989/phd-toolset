...
c_SolutionPresence::c_SolutionPresence(int id, int Certain, int Uncertain, int vc, bool tw) {
    srand(id);
...
//    TempDemand.clear();
//    TempDemand.push_back(2);
//    TempDemand.push_back(1);
//    TempDemand.push_back(0.5);
//    KD.push_back(TempDemand);
    ...
    for (int n = 1; n <= NodeAmount; n++) {
        ...
        NodeList[n].Stochastic = false;
        if (n  > Certain) {
            NodeList[n].Probability = 0.5;
            NodeList[n].Stochastic = true;
        }
...
c_SolutionDemand::c_SolutionDemand(int id, int Certain, int Uncertain, int vc, bool tw, int demandlevels) {
    srand(id);
    int NodeAmount = Certain + Uncertain;
    VehicleCapacity = vc;
    floatbigM = 999999999;
    ...
    for (int i = 0; i < demandlevels; i++) {
        vector<float> TempDemand;
        TempDemand.push_back(i+1);
        float tt1 = (float)(i+1)/demandlevels;
        TempDemand.push_back(tt1);
        TempDemand.push_back((float)1/demandlevels);
        KD.push_back(TempDemand);
    }
...
std::vector<NodeClass> c_SolutionCombo::ReadPCC6(string title) {
    ...
    while (infile.good()) // To get you all the lines.
    {
        ...
        Node.pcc6 = ss.str();// Prints our STRING.
        ss.str("");
        ...
        Node.LocationX = std::stof(ss.str());
        ss.str("");
        getline(infile, STRING);
        for (int i=0; i<STRING.length(); ++i)
        {
        ...
        Node.LocationY = std::stof(ss.str());
...
c_SolutionCombo::c_SolutionCombo(int id, int Certain, int Uncertain, int vc, bool tw, int demandlevels, string path, string X, string Y) {
    srand(id);
    int NodeAmount = Certain + Uncertain;
    VehicleCapacity = vc;
    floatbigM = 999999999;
    ...
    vector<NodeClass> allLoadedRequests = ReadPCC6(path + "NLpcc6 copy.txt");
    ...
    for (int i = 0; i < allLoadedRequests.size(); i++) {
        if (allLoadedRequests[i].LocationX > MaxX) {
            MaxX = allLoadedRequests[i].LocationX;
        }
        if (allLoadedRequests[i].LocationX < MinX) {
            MinX = allLoadedRequests[i].LocationX;
        }
        if (allLoadedRequests[i].LocationY > MaxY) {
            MaxY = allLoadedRequests[i].LocationY;
        }
        if (allLoadedRequests[i].LocationY < MinY) {
            MinY = allLoadedRequests[i].LocationY;
        }
    }
    
    avgY = ((MaxY - MinY) * 0.5) + MinY;
    avgX = ((54.9919513 - 48.04) * 0.5) + 48.04;
    ...
    for (int n = 1; n <= NodeAmount; n++) {
        ...
        while (added == false) {
            //Randomnumber from 0 to 100
            int randomrequest = (int)(allLoadedRequests.size() - 1);
            
            while (randomrequest >= (int)(allLoadedRequests.size() - 1)) {
                randomrequest = static_cast <int> (rand()) / (static_cast <int> (RAND_MAX / (int)(allLoadedRequests.size() - 1)));
            }
            
            //cout << allLoadedRequests[randomrequest].LocationX << ";" << allLoadedRequests[randomrequest].LocationY << endl;
            
            if (X == "U")
                critone = allLoadedRequests[randomrequest].LocationX > avgX;
            
            if (X == "D")
                critone = allLoadedRequests[randomrequest].LocationX < avgX;
            
            if (Y == "U")
                crittwo = allLoadedRequests[randomrequest].LocationY > avgY;
            ...
            if (critone
                && crittwo
                && allLoadedRequests[randomrequest].LocationX >= MinimumCoordX
                && allLoadedRequests[randomrequest].LocationX <= 54.9919513
                && RealNodeDistance(51.75559227, 5.261721656, allLoadedRequests[randomrequest].LocationX, allLoadedRequests[randomrequest].LocationY, drawRoute) < 99999999999) {
                    float x = allLoadedRequests[randomrequest].LocationX;
                    float y = allLoadedRequests[randomrequest].LocationY;
                    float demand = 1;//1 + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (5 - 1)));
                    float Probability = 1;
                    NodeList.push_back(NodeClass(x, y, demand, Probability, 0, 0, true));
                    NodeList[n].Stochastic = false;
                    NodeList[n].pcc6 = allLoadedRequests[randomrequest].pcc6;
                    if (n  > Certain) {
                        demand = demandlevels;
                        Probability = 0.5;
                        NodeList[n].Stochastic = true;
    ...
    FacilityList.clear();
    FacilityList.push_back(FacilityClass(51.75559227, 5.261721656));
    ...
    DistanceMatrix = GenerateRealDM(NodeList, FacilityList);
...
c_SolomonSolution::c_SolomonSolution(int id, const char* filename,  int Certain, int Uncertain, int vc, bool tw, int demandlevels) {
    srand(id);
    int NodeAmount = Certain + Uncertain;
    VehicleCapacity = vc;
    floatbigM = 999999999;
    ...
    NodeList.push_back(NodeClass(0, 0, 0,1,0,0, true));
    for (int n = id; n < id + Certain + Uncertain; n++) {
        float x = input.allCustomer[n].x_pos;
        float y = input.allCustomer[n].y_pos;
        float demand = input.allCustomer[n].demand;
        NodeList.push_back(NodeClass(x, y, demand,1, 0, 0, true));
        NodeList[n - id + 1].Probability = 1;
        vector<vector<float>> KD;
        NodeList[n - id + 1].Stochastic = false;
        for (int i = 0; i < demandlevels; i++) {
            vector<float> TempDemand;
            TempDemand.push_back(input.allCustomer[n].demand*(i+1));
            float tt1 = (float)(i+1)/demandlevels;
            TempDemand.push_back(tt1);
            TempDemand.push_back((float)1/demandlevels);
            KD.push_back(TempDemand);
        ...
        if (n  >= Certain + id) {
            demand = demandlevels * input.allCustomer[n].demand;
            NodeList[n - id + 1].Probability = 0.5;
            NodeList[n - id + 1].Stochastic = true;
        }
        NodeList[n - id + 1].StartTW = 0;
        NodeList[n - id + 1].ServiceTime = 0;
        NodeList[n - id + 1].DiscreteDemand = KD;
        NodeList[n - id + 1].EndTW = floatbigM;
        NodeList[n - id + 1].Service = true;
    ...
    for (int f = 0; f < 1; f++) {
        ...
        FacilityList[f].Capacity = 80;
    }
    
    FacilityAmount = 1;//(int)input.allFacilities.size();
...
c_CordeauSolution::c_CordeauSolution(int id, const char* filename) {
    srand(id);
    //NodesPerFacility = NPF;
    floatbigM = 999999999;
    intbigM = 999999999;
...
c_RealSolution::c_RealSolution(int id, int NPF, int vc) {
    ...
    ifstream myfileX("findroot.txt");
    cout << myfileX.is_open() << endl;
    
    ifstream myfileN("Nodes.txt");
    cout << myfileN.is_open() << endl;

	string line;
	ifstream fileGenesis("⁨findroot");
    cout << myfileX.is_open() << endl;
	while (it2 < 2967)
	{
		it += 1;
	...
	for (int n = 1; n <= 1000; n++) {
        ...
        if (prob < 0.95) {FacilityList[0].ServiceNodes.push_back(n);};
        if (prob >= 0.95 && prob < 0.975) {FacilityList[1].ServiceNodes.push_back(n);}
        if (prob >= 0.975) {FacilityList[2].ServiceNodes.push_back(n);}
	};

	for (int n = 1000 + 1; n <= 2 * 1000; n++) {
        float prob = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (1 - 0)));
        if (prob < 0.3) {FacilityList[0].ServiceNodes.push_back(n);};
        if (prob >= 0.3 /*&& prob < 1*/) {FacilityList[1].ServiceNodes.push_back(n);}
        //if (prob >= 0.85) {FacilityList[2].ServiceNodes.push_back(n);}
	};

	for (int n = 2 * 1000 + 1; n <= 2967; n++) {
        float prob = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (1 - 0)));
        if (prob < 0.3) {FacilityList[0].ServiceNodes.push_back(n);};
        if (prob >= 0.3 /*&& prob < 1*/) {FacilityList[2].ServiceNodes.push_back(n);}
        //if (prob >= 0.85) {FacilityList[2].ServiceNodes.push_back(n);}
	};
